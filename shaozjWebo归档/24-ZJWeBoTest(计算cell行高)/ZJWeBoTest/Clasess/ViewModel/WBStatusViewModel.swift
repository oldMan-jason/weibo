...
//       pictureSize = calculatePictureSize(count: model.pic_urls?.count)
    ...
    func updateRowHeight() -> Void
    {
        ...
        if let text = status.text {
           ...
           height += (text as NSString).boundingRect(with: viewSize, options: [.usesLineFragmentOrigin], attributes: [NSFontAttributeName: UIFont.systemFont(ofSize: 14)], context: nil).height
        ...
        if (status.retweeted_status != nil) {
            ...
            if let text = retweetedText {
               height += (text as NSString).boundingRect(with: viewSize, options: [.usesLineFragmentOrigin], attributes: [NSFontAttributeName: UIFont.systemFont(ofSize: 13)], context: nil).height
            }
