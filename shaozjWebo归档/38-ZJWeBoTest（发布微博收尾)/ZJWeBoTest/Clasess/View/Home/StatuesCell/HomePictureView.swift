...
class HomePictureView: UIView {
    ...
    var viewModel: WBStatusViewModel?{
        ...
        //设置配图视图的数据
        urlsModelArray = viewModel?.picUrls
      
        }
    }
    
    // MARK:- 修改配图视图的 高度  考虑单独和多图
    private func calcViewSize(){
        ...
        if viewModel?.picUrls?.count == 1 {
            //a 获取第0个视图
            let v  = subviews[0]
            v.frame = CGRect(x: wbstatusPictureInnerMatgin, y: wbstatusPictureInnerMatgin, width: viewSize.width, height: viewSize.height - wbstatusPictureOutterMargin)
        }else{
            //2>多图，保证九宫格
            let v  = subviews[0]
            v.frame = CGRect(x: wbstatusPictureInnerMatgin, y: wbstatusPictureInnerMatgin, width: wbstatusPictureItemWidth, height: wbstatusPictureItemWidth + wbstatusPictureOutterMargin)
        }
