...
class ComposeView: UIView {
    ...
    ///完成的闭包回调的记录属性
    var animationCompletionBlock:((_ clsName: String?)->())?
    ...
    // MARK:- 展示视图
    //OC中的block 当前的方法 不能执行，通常采用属性记录，需要的时候在调用
    func show(completion:@escaping (_ clsName: String?)->()) {
        
        //0 记录闭包
        animationCompletionBlock = completion
        
        //1 这个地方是添加在 根控制器的view上面
    ...
    func addButtons(view: UIView,idx : Int) -> Void {
        ...
        for i in idx..<(idx + count) {
            ...
            if let actionName = dict["actionName"] {
                ...
                btn.addTarget(self, action: #selector(clickButton(clickBtn:)), for: .touchUpInside)
            }
        }
...
//        scrollView.contentOffset = offset
    ...
    func clickButton(clickBtn: ComposeItemView) -> Void {
...
//            scaleAnim.toValue = scale
            ...
            //2> 渐变动画 - 动画组
            let alpAnimation : POPBasicAnimation = POPBasicAnimation(propertyNamed: kPOPViewAlpha)
            alpAnimation.toValue = 0.2
            alpAnimation.duration = 0.5
            btn.pop_add(alpAnimation, forKey:nil)
            
            //3> 监听动画的完成 顺便 设置判断一个条件，因为时间都是相同的
            if i == 0 {
                
                scaleAnim.completionBlock = {
                    _,_ in
                    print("动画完成")
                    //执行闭包
                    self.animationCompletionBlock?(clickBtn.btnClassName)
