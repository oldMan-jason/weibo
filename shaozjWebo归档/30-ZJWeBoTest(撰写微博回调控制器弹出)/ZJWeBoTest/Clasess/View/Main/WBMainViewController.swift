...
extension WBMainViewController {
    ...
    func composeStatus() -> Void {
        ...
        //点击某个按钮的回调，然后弹出视图控制器  这里有循环引用
        composeView.show { [weak composeView] (clsName) in
            
            guard let clsName = clsName ,let spaceName = Bundle.main.infoDictionary?["CFBundleName"] as? String else{
            ...
            guard let vcCls = NSClassFromString(spaceName + "." + clsName) as? WBComposeViewController.Type else{
                return
            }
