...
class WBComposeViewController: UIViewController {
    ...
    //注意 协议代理是通过 xib拖线设置的
    @IBOutlet weak var textView: WBTextView!//文本框
    
    @IBOutlet weak var toolBar: UIToolbar!//底部工具栏
    
    @IBOutlet weak var toolBarBottomCons: NSLayoutConstraint!
    
    lazy var keyBoardInputView :KBInputView = KBInputView.setupInputView {[weak self] (emodel) in
        
            self?.textView.insertTextViewEmotion(em: emodel)
    }
...
extension WBComposeViewController{
    ...
    func emoticonKeyboard() -> Void {
        ...
        // 设置键盘视图
        textView.inputView = (textView.inputView == nil) ? keyBoardInputView : nil
        //！！！！刷新键盘视图
